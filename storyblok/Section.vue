...
<template>
  ...
  <section v-editable="blok" :style="backgroundStyle" :class="{ 'py-16': !!blok?.padding }"
    ...
    <div :class="blok?.layout" :data-aos="blok?.animate" data-aos-delay="500" data-aos-once="true">
      <h2 v-if="!!blok?.title"
        class="font-semibold flex items-center mb-5 font-heading text-gray-800 dark:text-gray-100"
        :style="{ color: blok.title_color?.color }" :class="{ 'text-center justify-center': blok?.text_center }">
...
.prose {
  max-width: 100%;
  /* Ensure prose content does not exceed viewport width */
}

.flex.items-center {
  max-width: 100%;
  /* Ensure flex container does not exceed viewport width */
}
</style>
